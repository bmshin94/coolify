...
## 프로젝트 개요
비싼 클라우드 구독료를 매달 내지 않고 내 컴퓨터나 저렴한 가상 서버를 나만의 프라이빗 배포 플랫폼으로 바꿔주는 "오픈소스 셀프호스팅 PaaS"
클릭 몇 번으로 데이터베이스, 웹사이트, 오픈소스 애플리케이션을 자동으로 설치하고 완벽하게 운영 및 백업
클라우드 비용을 90% 이상 절감하면서도 대기업급 편리함을 내 손안에서 누리고 싶은 개발자와 스타트업의 필수품
